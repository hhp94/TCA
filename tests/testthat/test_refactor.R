test_that("Compare the result of refactor with those of the matlab version of refactor", {
  ...
  rownames(X) <- 1:nrow(X)
  C <- as.matrix(read.table(assets_p("C2.txt"), header = FALSE, sep = ","))
  t <- 50
  k <- 3
  matlab.ref_comp <- as.matrix(read.table(assets_p("refactor.R_est.txt"), header = FALSE, sep = ","))
  matlab.ranked_list <- as.matrix(read.table(assets_p("refactor.ranked_list.txt"), header = FALSE, sep = ","))
  matlab.ref_comp.adj <- as.matrix(read.table(assets_p("refactor.R_est.adjusted.txt"), header = FALSE, sep = ","))
  matlab.ref_comp.adj.C.remove <- as.matrix(read.table(assets_p("refactor.R_est.adjusted.C.remove.txt"), header = FALSE, sep = ","))
  matlab.ranked_list.adj <- as.matrix(read.table(assets_p("refactor.ranked_list.adjusted.txt"), header = FALSE, sep = ","))
