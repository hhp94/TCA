...
* Replace this pattern `repmat(W_norms[, j],` with `MESS::repmat(matrix(W_norms[, j], nrow = 1),`
...
repmat_opt <- readRDS("assets/repmat_optimize.rds")
waldo::compare(
  t(repmat(repmat_opt$a, repmat_opt$n, repmat_opt$m)),
  MESS::repmat(matrix(repmat_opt$a), repmat_opt$m, repmat_opt$n)
)
waldo::compare(
  pracma::repmat(repmat_opt$a, 1000, 1),
  MESS::repmat(matrix(repmat_opt$a, nrow = 1), 1000, 1)
)
...
microbenchmark::microbenchmark(
  pracma::repmat(repmat_opt$a, 1, 1000),
  MESS::repmat(matrix(repmat_opt$a, nrow = 1), 1, 1000),
  times = 1000
)
```
