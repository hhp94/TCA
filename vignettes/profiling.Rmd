...
memory usage compared to running `TCA::tca()` version 1.2.1 sequentially.
...
`TCA::tca()` in parallel over the independent chunks of X. See (vignettes/tca_split.pdf)
...
machine tolerance of differences from the test fixtures
...
estimates within machine tolerance of difference from the estimates of the 
fit produced by version 1.2.1.  
...
This is potentially an unsafe change. But the risk is very small because
  ...
  with more safety checks and probably is only slightly slower than `fastLm()`.
...
* Fixed bugs in `split_input()`. Added unit tests for `split_input()` and 
`tca_split()`
...
except for the `tcareg()` related functions (because `tcareg()` is fast enough).
