...
```{r}
# tca.fit
# saveRDS(list(a = W_norms[, j], n = k, m = 1), "assets/repmat_1imize.rds")
# stop()
...
waldo::compare(
  apply(repmat_2$X_tilde, 2, function(v) {repmat(v, 1, 3)}),
  apply(repmat_2$X_tilde, 2, function(v) {MESS::repmat(matrix(v), ncol = 3)})
)
```
