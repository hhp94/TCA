...
scenarios  
* Profile with `profvis::profvis()` and identify bottle necks  
...
with memory usage by 2x. This is potentially an unsafe change. But the 
risk is small because.  
  ...
  the optimization throw an error. This shouldn't be necessary.  
  ...
  formatting of with `stats::anova()` calls. Implementing a minimal version of 
  the partial F-test also helped reduced fit time.  
  + We can also use `lm.fit()` as well to minimize dependencies. `lm.fit()` comes 
  with more safety checks and probably no significance different performance
  compared to `fastLm()`.  
...
fit one final fit with `vars.mle = TRUE`.  
...
will give back the same estimates using `{testthat}`.  

# Best practices
* In short, the correct way to use TCA is to not use the parallel argument in
`TCA::tca()`.  
  + On a Windows machine, the overhead caused by data being shuffled back and 
...
* The correct way to parallel `TCA::tca()` is to chunk the data by X and run 
parallel over each chunk instead.  
  + For example, for 400,000 CpGs, chunk into 7 x 50,000\*m chunks or 
...
outside of the `tcareg()` related functions. This shall be tackled later.  
...
prof_obj <- profvis({tca(X = data$X, W = data$W, C1 = data$C1, C2 = data$C2)})
...
prof_obj <- profvis({
  tca(X = data$X, W = data$W, C1 = data$C1, C2 = data$C2, parallel = TRUE, 
      num_cores = 6L)
})
