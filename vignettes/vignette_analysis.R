...
if (FALSE) {
  ...
  sprintf(
    "Detected %d associations using a joint test, %d associations using a marginal conditional test, and %d associations in CD4 cells using a marginal conditional test.",
  ...
  cd4.hits.liu.pvals <- unlist(lapply(
    1:length(cd4.hits),
  ...
  liu.cd4.regression.pvals <- unlist(lapply(
    1:nrow(liu.cd4$X),
  ...
  tcareg.mdl.paquette.joint <- tcareg(
    X = paquette$X,
  ...
  tca.mdl.paquette.refit_W <- tca(
    X = paquette$X.ref_cpgs,
    W = paquette$W,
    C1 = paquette$cov[, c("gender", "gestational_age")],
    C2 = paquette$cov[, 4:ncol(paquette$cov)],
    constrain_mu = TRUE,
  ...
  # Run tcareg with a joint test and extract p-values; generate a qq-plot
  tcareg.mdl.paquette.joint.2 <- tcareg(
    X = paquette$X,
  ...
  tcareg.mdl.paquette.2.marg_cond <- tcareg(
    X = paquette$X,
    tca.mdl = tca.mdl.paquette.2,
    y = paquette$cov[, "arousal", drop = F],
    C3 = paquette$cov[, C3_names],
  ...
  tcasub.mdl.paquette.2 <- tcasub(
    tca.mdl = tca.mdl.paquette.2,
    features = hit.joint
  )

  # Calculate cell-type-specific methylation for the samples in the detected CpG
  tensor.mdl.hit <- tensor(
    tca.mdl = tcasub.mdl.paquette.2,
